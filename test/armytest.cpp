...
namespace cSzd
{
    ...
    // ---------------- generateLegalMoves() testing (More on ChessBoard...) -------------------------
    // --- generateKingLegalMoves() method Testing
    TEST(ArmyTester, CheckLegalMovesOfKingsInE5AndA1WithNoInterference)
    {
        Army b{};
        b.color = BlackArmy;
        b.pieces[King] = BitBoard(e5);

        Army w{};
        w.color = WhiteArmy;
        w.pieces[King] = BitBoard(a1);

        std::vector<ChessMove> blackKingMoves;
        b.generateKingLegalMoves(blackKingMoves, w);
        ...
        w.generateKingLegalMoves(whiteKingMoves, b);
    ...
    TEST(ArmyTester, CheckLegalMovesOfKingsInB4AndD5WithSomeInterferenceAndCaptures)
    {
        Army w{};
        w.color = WhiteArmy;
        w.pieces[King] = BitBoard(b4);
        w.pieces[Queen] = BitBoard(b2);
        w.pieces[Knight] = BitBoard(e6);
        Army b{};
        b.color = BlackArmy;
        b.pieces[King] = BitBoard(d5);
        b.pieces[Rook] = BitBoard(h3);
        b.pieces[Pawn] = BitBoard(b5);
