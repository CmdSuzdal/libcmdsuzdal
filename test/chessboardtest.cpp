...
namespace cSzd
{
    ...
    TEST(ChessBoardTester, NotationToMove_EmptyStringGeneratesInvalidMove)
    {
        ChessBoard cb {FENInitialStandardPosition};
        ASSERT_EQ(cb.notation2Move(""), InvalidMove);
    }
    TEST(ChessBoardTester, NotationToMove_FromInitialPosition_e4_Generates_Pawn_e2_e4_Move)
    {
        ChessBoard cb {FENInitialStandardPosition};
        ASSERT_EQ(cb.notation2Move("e4"), chessMove(Pawn, e2, e4));
    }
    TEST(ChessBoardTester, NotationToMove_FromInitialPosition_d3_Generates_Pawn_d2_d3_Move)
    {
        ChessBoard cb {FENInitialStandardPosition};
        ASSERT_EQ(cb.notation2Move("d3"), chessMove(Pawn, d2, d3));
    }
    TEST(ChessBoardTester, NotationToMove_FromInitialPosition_g1_Generates_InvalidMove)
    {
        ChessBoard cb {FENInitialStandardPosition};
        ASSERT_EQ(cb.notation2Move("g1"), InvalidMove);
    }
    TEST(ChessBoardTester, NotationToMove_FromASicilianVariantPosition_WhiteToMove_ConvertPawnMoves)
    {
    ...
    TEST(ChessBoardTester, NotationToMove_FromASicilianVariantPosition_BlackToMove_ConvertPawnMoves)
    {
        ChessBoard cb {"r2q1rk1/1p1nbppp/p2pbn2/4p3/4P1P1/1NN1BP2/PPPQ3P/2KR1B1R b - - 0 11"};
        ASSERT_EQ(cb.notation2Move("d5"), chessMove(Pawn, d6, d5));
        ASSERT_EQ(cb.notation2Move("g5"), chessMove(Pawn, g7, g5));
